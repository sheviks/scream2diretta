...
int run_pcap(int (*output_function)(receiver_data_t*))
{
  ...
  // pcap_dispatch returns after a short batch (limited by the timeout passed
  // to pcap_open_live) so we can poll g_shutdown_pending between batches.
  // Using an unbounded pcap_loop would swallow SIGINT on libpcap builds that
  // don't expose a signal-safe break.
  while (!g_shutdown_pending) {
    int n = pcap_dispatch(handle, 64, pcap_callback, NULL);
    if (n < 0) {
      // -1 = error, -2 = pcap_breakloop()
      break;
    }
  }
